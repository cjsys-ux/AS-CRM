...
function ImageUploadField({ imageUrl, onImageChange }: { imageUrl: string; onImageChange: (url: string) => void }) {
  ...
  const handleUpload = useCallback(async (file: File) => {
    ...
    setUploading(true);
    try {
      // Read file as base64 for the local /api/files/upload JSON contract.
      ...
      const response = await fetch('/api/files/upload', {
        ...
        body: JSON.stringify({
          fileName: `inventory-${Date.now()}-${file.name}`,
          fileType: file.type || 'image/png',
          entityType: 'inventory-image',
          entityId: 'inventory',
          fileData: base64,
        }),
      });
      if (!response.ok) { toast.error('Upload failed'); return; }
      const data = await response.json();
      const url = data.fileUrl || (data.key ? `/api/files/image?key=${encodeURIComponent(data.key)}` : null);
      if (url) {
        onImageChange(url);
...
export function AddInventoryItemDrawer({ isOpen, onClose, editItem, onSuccess, onSave }: AddInventoryItemDrawerProps) {
    ...
    const fetchVendors = async () => {
      ...
      try {
        ...
        const opts = (data.vendors || []).map((v: any) => ({
          label: v.name || v.vendorName || v.company || v.id,
          value: v.name || v.vendorName || v.company || v.id,
          subtitle: v.category || v.type || v.vendorType || v.email || '',
        }));
        setVendors(opts);
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEdit && editItem) {
        response = await fetch('/api/inventory/update', {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ id: editItem.id, ...payload }),
        });
      } else {
        response = await fetch('/api/inventory/create', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      if (response.ok) {
        ...
        toast.error(`Failed to ${isEdit ? 'update' : 'add'} item: ${data.error || 'Unknown error'}`);
      }
    } catch (error) {
