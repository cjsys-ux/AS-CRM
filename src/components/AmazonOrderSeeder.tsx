    ...
    for (let i = 0; i < sorted.length; i += BATCH) {
      ...
      await Promise.all(
        batch.map(async (raw) => {
          ...
          try {
            const res = await fetch('/api/amazon-orders/create', {
              method: 'POST',
          ...
          >
            ...
            <div className="p-6">
              ...
              <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-200 mb-5">
                <div className="w-16 h-16 rounded-xl border border-slate-200 bg-white p-1 flex items-center justify-center">
