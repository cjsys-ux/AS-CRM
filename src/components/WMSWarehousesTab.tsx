...
export function WMSWarehousesTab() {
  ...
  const fetchLocations = useCallback(async (_warehouseId: string) => {
    setLocations([]);
  }, []);
  ...
  const handleSaveWarehouse = async (data: any) => {
    if (editWarehouse) {
      setWarehouses(prev => prev.map(item => item.id === editWarehouse.id ? { ...item, ...data } : item));
  ...
  const handleGenerateLocations = async (locs: any[]) => {
    ...
    const newLocs = locs.map((l, i) => ({ ...l, id: (Date.now() + i).toString(), warehouseId: selectedWarehouse.id }));
    setLocations(prev => [...prev, ...newLocs]);
    toast.success(`Generated ${locs.length} locations!`);
  };
  ...
  const handleBulkDelete = async () => {
    ...
    setLocations(prev => prev.filter(item => !selectedIds.has(item.id)));
    toast.success(`Deleted ${count.toLocaleString()} locations!`);
    setSelectedIds(new Set());
