...
export function WMSWarehousesTab() {
  ...
  const fetchWarehouses = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/wms/warehouses', { headers });
  ...
  const fetchLocations = useCallback(async (warehouseId: string) => {
    try {
      const res = await fetch(`/api/warehouse-locations/${warehouseId}`, { headers });
      const data = await res.json();
  ...
  const handleSaveWarehouse = async (data: any) => {
    try {
      ...
      const url = editWarehouse ? `/api/warehouses/${editWarehouse.id}` : '/api/wms/warehouses';
  ...
  const handleGenerateLocations = async (locs: any[]) => {
    ...
    try {
      ...
      for (let i = 0; i < locs.length; i += 50) {
        ...
        await fetch(`/api/warehouse-locations/${selectedWarehouse.id}`, {
  ...
  const handleDeleteLocation = async (loc: WarehouseLocation) => {
    try {
      const res = await fetch(`/api/warehouse-locations/${loc.warehouseId}/${loc.id}`, { method: 'DELETE', headers });
      const data = await res.json();
      if (data.success) {
