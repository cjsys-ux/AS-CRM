...
export function AddProductDatabaseDrawer({ isOpen, onClose, productData, onSuccess }: AddProductDatabaseDrawerProps) {
  ...
  useEffect(() => {
    if (!isOpen) return;
    const fallback = ['Screen Print', 'Pad Print', 'Full Color', 'Laser Engrave', 'Embroidery', 'Heat Transfer', 'Sublimation', 'Deboss', 'UV Print', 'DTF'];
    fetch('/api/settings/product-database/get')
      .then(res => res.ok ? res.json() : Promise.reject())
      .then(data => {
        const settings = data.settings ?? {};
        const methods = settings['decoration-methods'] || settings.decorationMethods;
        setDecorationMethods(Array.isArray(methods) && methods.length ? methods : fallback);
      })
      .catch(err => {
        console.error('Error fetching decoration methods:', err);
        setDecorationMethods(fallback);
      });
  }, [isOpen]);
  ...
  const uploadImageToStorage = async (base64: string): Promise<string> => {
    if (!base64 || base64.startsWith('http') || base64.startsWith('/api/files/')) return base64;
    try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (productData?.id) {
        const response = await fetch('/api/productdb/update', {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ id: productData.id, ...product }),
        });
        if (!response.ok) {
        ...
        const response = await fetch('/api/productdb/create', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
