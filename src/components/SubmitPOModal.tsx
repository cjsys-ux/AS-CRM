...
export function SubmitPOModal({ isOpen, onClose, onConfirm, onSubmitAndConfirm, poNumber }: SubmitPOModalProps) {
  ...
  // Reset the order number question when method changes
  useEffect(() => {
  ...
  const handleSubmit = () => {
    ...
    // If online + received order number → submit AND confirm in one step
    if (submitMethod === 'online' && receivedOrderNumber === true) {
      ...
      if (onSubmitAndConfirm) {
        onSubmitAndConfirm(methodLabels[submitMethod].label, details, confirmationNumber);
      } else {
        // Fallback: just do regular submit with confirmation info in details
        onConfirm(methodLabels[submitMethod].label, `${details} | Confirmation #: ${confirmationNumber}`);
      }
      return;
    }
