...
export function WMSReceivingTab({ onNavigate }: { onNavigate?: (page: string) => void }) {
  ...
  const handleSave = async (data: any) => {
    ...
    setReceipts(prev => [...prev, newItem]);
    toast.success('Inbound shipment created!');
    setDrawerOpen(false);
  };

  const handleUpdate = async (updated: ReceivingItem) => {
    setReceipts(prev => prev.map(item => item.id === updated.id ? { ...item, ...updated } : item));
    toast.success(updated.status === 'Completed' ? 'Receiving completed!' : 'Progress saved!');
  };
  ...
  const handleBulkDelete = async () => {
    setReceipts(prev => prev.filter(item => !selectedRows.includes(item.id)));
