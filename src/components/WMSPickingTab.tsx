...
export function WMSPickingTab() {
  ...
  const handleSave = async (data: any) => {
    try {
      const res = await fetch(`${API_URL}/pick-lists`, { method: 'POST', headers, body: JSON.stringify(data) });
  ...
  const handleUpdate = async (updated: PickList) => {
    try {
      await fetch(`${API_URL}/pick-lists/${updated.id}`, { method: 'PUT', headers, body: JSON.stringify(updated) });
      ...
      if (updated.status === 'Completed' && (updated as any).sourceType === 'inventory-shipment' && (updated as any).sourceInventoryId) {
        try {
          ...
          const invRes = await fetch(`${API_URL}/inventory/${invId}`, { headers });
          ...
          if (invData.success && invData.item) {
            ...
            await fetch(`${API_URL}/inventory/${invId}`, {
              method: 'PUT',
              headers,
  ...
  const handleDelete = async (id: string) => {
    try { await fetch(`${API_URL}/pick-lists/${id}`, { method: 'DELETE', headers }); toast.success('Deleted'); fetchPickLists(); }
