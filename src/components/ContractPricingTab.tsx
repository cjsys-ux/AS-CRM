...
export function ContractPricingTab({ vendorId, vendorName }: ContractPricingTabProps) {
  ...
  const saveDecTypeConfig = async (_types: string[]) => {
    // local state only
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const savedSheet: PricingSheet = { vendorId, decorationType: activeDecType, year: activeYear, ...payload } as PricingSheet;
      setAllSheets(prev => ({ ...prev, [sheetKey]: savedSheet }));
      setHasChanges(false);
    } catch (err) {
