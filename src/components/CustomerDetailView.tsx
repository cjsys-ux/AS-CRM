...
export function CustomerDetailView({ customerId, onBack, onCustomerUpdated }: CustomerDetailViewProps) {
  ...
  const fetchCustomerDetails = async () => {
    setIsLoading(true);
    setCustomer({ id: customerId, name: 'Customer', logo: '', industry: '', size: '', status: 'Active', resaleCert: '', website: '', spend: 0 });
    setContacts([]);
    setAddresses([]);
    setDocuments([]);
    setIsLoading(false);
  ...
  const handleAddNote = async () => {
    ...
    const newNoteObj = { id: `note-${Date.now()}`, text: newNote, author: 'You', timestamp: new Date().toISOString() };
    setPersistedNotes([newNoteObj, ...persistedNotes]);
    setNewNote('');
  ...
  const handleCreateInvoice = async () => {
    const newInvoice = { ...invoiceForm, id: `inv-${Date.now()}`, amount: parseFloat(invoiceForm.amount) || 0 };
  ...
  const handleAddAddress = async (address: Address) => {
    ...
    const base = address.isPrimary ? addresses.map(a => ({ ...a, isPrimary: false })) : addresses;
    setAddresses([...base.filter(a => !address.isPrimary || !a.isPrimary), newAddress]);
  ...
  const handleFileUpload = async (file: File, fileName: string, fileType: string) => {
    ...
    const newFile = { id: `file-${Date.now()}`, name: fileName, type: fileType, size: file.size, uploadedAt: new Date().toISOString() };
