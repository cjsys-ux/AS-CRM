...
export function ContactDetailView({ contact, onBack, onDelete, onContactUpdated }: ContactDetailViewProps) {
  ...
  const fetchContact = useCallback(async () => {
    try {
      const res = await fetch(`/api/contacts/get?id=${encodeURIComponent(contact.id)}`);
      if (!res.ok) return;
  ...
  const fetchDocuments = useCallback(async () => {
    setLoadingDocs(true);
    try {
      const res = await fetch(`/api/contacts/documents/list?contactId=${encodeURIComponent(contact.id)}`);
      if (res.ok) {
        const data = await res.json();
  ...
  const fetchActivity = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/contacts/activity/list?contactId=${encodeURIComponent(contact.id)}`);
      if (res.ok) {
        const data = await res.json();
  ...
  const fetchEmails = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/contacts/emails/list?contactId=${encodeURIComponent(contact.id)}`);
      if (res.ok) {
        const data = await res.json();
  ...
  const fetchTickets = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/contacts/tickets/list?contactId=${encodeURIComponent(contact.id)}`);
      if (res.ok) {
        const data = await res.json();
  ...
  const handleSendEmail = async () => {
    ...
    try {
      const res = await fetch('/api/contacts/emails/create', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ contactId: contact.id, ...emailForm, to: contactData.email }),
      });
      if (res.ok) {
  ...
  const handleCreateTicket = async () => {
    ...
    try {
      const res = await fetch('/api/contacts/tickets/create', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ contactId: contact.id, ...ticketForm }),
      });
      if (res.ok) {
  ...
  const handleUpdateTicketStatus = async (ticketId: string, newStatus: string) => {
    try {
      await fetch('/api/contacts/tickets/update', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const saveAddress = async () => {
    ...
    try {
      const res = await fetch('/api/contacts/update', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: contact.id, addresses: newAddresses }),
      });
  ...
  const handleFileUpload = async () => {
    ...
    for (const file of uploadFiles) {
      try {
        ...
        // 1. Upload file bytes to S3 via the presign/complete flow.
        const presignRes = await fetch('/api/files/presign', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ fileName: file.name, fileType: file.type, entityType: 'contact-document', entityId: contact.id }),
        });
        ...
        await fetch('/api/files/complete', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ key, fileName: file.name, fileType: file.type, size: file.size, entityType: 'contact-document', entityId: contact.id, uploadedBy: 'Current User' }),
        });
        // 2. Persist a document row referencing the S3 key.
        await fetch('/api/contacts/documents/create', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ contactId: contact.id, name: file.name, type: file.type, size, fileKey: key, fileUrl: fileUrl ?? null, uploadedBy: 'Current User' }),
        });
      } catch (error) {
  ...
  const confirmDeleteDoc = async () => {
    if (deleteDoc) {
      try {
        await fetch('/api/contacts/documents/delete', {
          method: 'DELETE',
          headers: { 'Content-Type': 'application/json' },
