...
export function ContactDetailView({ contact, onBack, onDelete, onContactUpdated }: ContactDetailViewProps) {
  ...
  const fetchContact = useCallback(async () => {
    setContactData(contact as any);
    setAddresses((contact as any).addresses || []);
  }, [contact.id]);
  ...
  const handleSendEmail = async () => {
    ...
    const newEmail = { id: `email-${Date.now()}`, ...emailForm, to: contactData.email, sentAt: new Date().toISOString() };
  ...
  const handleCreateTicket = async () => {
    ...
    const newTicket = { id: `ticket-${Date.now()}`, ...ticketForm, status: 'Open', createdAt: new Date().toISOString() };
  ...
  const handleFileUpload = async () => {
    ...
    const newDocs = uploadFiles.map(file => {
      const size = file.size > 1024 * 1024 ? `${(file.size / (1024 * 1024)).toFixed(1)} MB` : `${(file.size / 1024).toFixed(0)} KB`;
      return { id: `doc-${Date.now()}-${file.name}`, name: file.name, size, uploadedBy: 'Current User', uploadedAt: new Date().toISOString() };
    });
  ...
  const confirmDeleteDoc = async () => {
    if (deleteDoc) {
      setUploadedFiles(prev => prev.filter(f => f.id !== deleteDoc.id));
      setDeleteDoc(null);
    }
  };
              ...
              <div className="p-6 space-y-4 overflow-y-auto flex-1">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input type="text" placeholder="CA" value={addressForm.state} onChange={e => setAddressForm({ ...addressForm, state: e.target.value })} className="w-full px-4 py-2.5 bg-slate-50 border-2 border-slate-200 rounded-xl text-sm font-medium text-slate-900 focus:outline-none focus:ring-2 focus:ring-amber-500/30 focus:border-amber-500 transition-all" />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
