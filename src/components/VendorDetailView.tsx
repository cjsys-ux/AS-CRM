...
export function VendorDetailView({ vendor, onBack, onDelete, onVendorUpdated }: VendorDetailViewProps) {
  ...
  const saveVendorField = async (updates: Record<string, any>) => {
    setSaving(true);
    try {
      const updated = { ...vendorData, ...updates };
  ...
  const handleUpload = async () => {
    ...
    try {
      for (const file of uploadFiles) {
        ...
        const newDoc = { id: `doc-${Date.now()}-${file.name}`, name: docName, type: resolvedType, size: sizeStr, preview, uploadedBy: 'Current User', vendorId: vendor.id };
  ...
  const handleSaveContact = async () => {
    ...
    try {
      ...
      if (editingContact) {
        setVendorContacts(prev => prev.map(c => c.id === editingContact.id ? { ...c, ...contactForm, name: fullName } : c));
        toast.success('Contact updated');
      } else {
        const newContact = { ...contactForm, name: fullName, id: `contact-${Date.now()}` } as VendorContact;
  ...
  const handleDeleteContact = async (contact: VendorContact) => {
    setVendorContacts(prev => prev.filter(c => c.id !== contact.id));
