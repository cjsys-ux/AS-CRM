...
export function OrdersPage({ onNavigate }: { onNavigate?: (page: string) => void }) {
  ...
  const syncOrderTotalsFromPOs = async (_ordersList: Order[]) => {
    // no-op in local mode
  };
          ...
          <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-lg">
            ...
            <div className="flex items-center gap-3 mt-4">
              ...
              <motion.button
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                className="flex items-center gap-2 px-4 py-2.5 bg-blue-500 text-white font-semibold rounded-xl hover:bg-blue-600 transition-all text-sm ml-auto"
              >
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[1500px]">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    {isColVisible('orderId') && <th className="text-left px-4 py-4 text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">Order ID</th>}
                    ...
                    {isColVisible('items') && <th className="text-left px-4 py-4 text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">Items</th>}
                    ...
                    {isColVisible('date') && <th className="text-left px-4 py-4 text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">Date</th>}
                ...
                <tbody className="divide-y divide-slate-100">
                  {loading ? (
                    ...
                    paginatedOrders.map((order, index) => (
                      ...
                      >
                        {isColVisible('orderId') && <td className="px-4 py-4 whitespace-nowrap">
                        ...
                        {isColVisible('customer') && <td className="px-4 py-4 whitespace-nowrap">
                        ...
                        {isColVisible('projectNumber') && <td className="px-4 py-4 whitespace-nowrap">
                          {order.projectNumber ? (
                        ...
                        {isColVisible('project') && <td className="px-4 py-4 whitespace-nowrap">
                          <span className="text-sm text-slate-700">{getProjectName(order) || '—'}</span>
                        </td>}
                        {isColVisible('poNumber') && <td className="px-4 py-4 whitespace-nowrap">
                        ...
                        {isColVisible('items') && <td className="px-4 py-4 whitespace-nowrap">
                          <span className="text-sm text-slate-700">
                            {getItemQuantity(order)} {getItemQuantity(order) === 1 ? 'item' : 'items'}
                          </span>
                        </td>}
                        {isColVisible('shipDate') && <td className="px-4 py-4 whitespace-nowrap">
                          <span className="text-sm text-slate-600">{order.shipDate || '—'}</span>
                        </td>}
                        {isColVisible('inHands') && <td className="px-4 py-4 whitespace-nowrap">
                          {order.inHandsDate ? (
                        ...
                        {isColVisible('date') && <td className="px-4 py-4 whitespace-nowrap">
                          <span className="text-sm text-slate-600">{getOrderDate(order)}</span>
                        </td>}
                        {isColVisible('total') && <td className="px-4 py-4 whitespace-nowrap">
