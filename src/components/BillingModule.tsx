...
export function BillingModule() {
  ...
  const fetchAmazonOrders = useCallback(async () => {
    try {
      const res = await fetch('/api/amazon-orders/list');
      if (res.ok) {
  ...
  return (
    <div className="p-4 space-y-4 max-w-full">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-lg font-bold text-slate-900 flex items-center gap-2">
            ...
            Billing & Collections
            <span
              className="ml-1 px-2 py-0.5 bg-slate-100 rounded-full text-[10px] font-semibold text-slate-500 flex items-center gap-1">
              <Sparkles className="w-3 h-3" /> AI-Powered
            </span>
          </h1>
          <p className="text-xs text-slate-500 mt-0.5">Manage invoices, track payments, and generate collection statements</p>
        </div>
        <div className="flex gap-2">
          <button onClick={() => { setLoading(true); Promise.all([fetchAmazonOrders(), fetchCorporateInvoices()]).finally(() => setLoading(false)); }}
            className="px-2.5 py-1.5 text-xs font-medium text-slate-600 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 flex items-center gap-1.5 transition-all">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
        className="flex bg-slate-100 rounded-xl p-1 w-fit">
        {[
          ...
          <button key={tab.id} onClick={() => setActiveSection(tab.id)}
            className={`relative px-4 py-2 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-all duration-300 ${
              activeSection === tab.id ? 'text-white shadow-lg' : 'text-slate-500 hover:text-slate-700'
            }`}>
            {activeSection === tab.id && (
              <motion.div layoutId="billing-tab" className="absolute inset-0 bg-slate-800 rounded-lg" />
            )}
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <KPICard label="Total Amazon Revenue" value={`$${fmt(totals.totalAmazonRevenue)}`}
          icon={DollarSign} color="bg-emerald-500" delay={0} trend="up" />
        <KPICard label="Total Cost" value={`$${fmt(totals.totalCost)}`}
          icon={CreditCard} color="bg-blue-500" delay={0.05} />
        <KPICard label="Total Profit" value={`$${fmt(totals.totalProfit)}`}
          icon={TrendingUp} color="bg-purple-500" delay={0.1} trend="up" />
        <KPICard label="Balance Due" value={`$${fmt(amountDue.totalBalanceWithProfit)}`}
          icon={AlertTriangle} color="bg-amber-500" delay={0.15}
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
        ...
        <div className="bg-slate-800 px-4 py-3 flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-sm font-bold text-white">Total Amount Due on</h2>
              <p className="text-slate-400 text-[10px]">Cost & profit breakdown</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <div className="bg-white text-slate-900 px-3 py-1.5 rounded-lg text-xs font-semibold shadow-sm">
              {displayDate}
            </div>
            <input
              type="date"
              ...
              className="px-2.5 py-1.5 rounded-lg text-xs bg-white/10 text-white border border-white/20 focus:outline-none focus:ring-2 focus:ring-white/30 cursor-pointer"
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                <th className="text-left px-3 py-2 font-semibold text-slate-500 text-[10px] uppercase tracking-wider whitespace-nowrap"></th>
                <th className="text-right px-3 py-2 font-semibold text-slate-500 text-[10px] uppercase tracking-wider whitespace-nowrap">Amount</th>
                <th className="text-right px-3 py-2 font-semibold text-slate-500 text-[10px] uppercase tracking-wider whitespace-nowrap">Total Cost</th>
                <th className="text-right px-3 py-2 font-semibold text-slate-500 text-[10px] uppercase tracking-wider whitespace-nowrap">Cost & Profit</th>
                <th className="text-right px-3 py-2 font-semibold text-slate-500 text-[10px] uppercase tracking-wider whitespace-nowrap">Payments</th>
                <th className="text-right px-3 py-2 font-semibold text-slate-500 text-[10px] uppercase tracking-wider whitespace-nowrap">Balance Due</th>
                <th className="text-right px-3 py-2 font-semibold text-slate-500 text-[10px] uppercase tracking-wider whitespace-nowrap min-w-[160px]">Total Balance Due w/ Profit</th>
                <th className="text-right px-3 py-2 font-semibold text-slate-500 text-[10px] uppercase tracking-wider whitespace-nowrap min-w-[150px]">Total Cost Only Due</th>
              </tr>
            </thead>
            <tbody>
              {[
                ...
                <tr key={row.label}
                  ...
                  <td className="px-3 py-2 text-xs font-medium text-slate-700 whitespace-nowrap">{row.label}</td>
                  <td className="px-3 py-2 text-right text-xs font-mono text-slate-700 whitespace-nowrap">$ {fmt(row.amount)}</td>
                  ...
                  <td className="px-3 py-2 text-right text-xs font-mono text-emerald-600 whitespace-nowrap">
                    {row.payments !== null ? `$ ${fmt(row.payments)}` : ''}
                  </td>
                  <td className="px-3 py-2 text-right text-xs font-mono font-medium text-red-600 whitespace-nowrap">
                    $ {fmt(row.balanceDue)}
                  </td>
                  {row.isFirst ? (
                    <>
                      <td rowSpan={4} className="px-3 py-2 text-center align-middle">
                        <div className="inline-flex flex-col items-center bg-slate-50 border border-slate-200 rounded-lg px-4 py-2.5">
                          <span className="text-[9px] font-semibold text-slate-400 uppercase tracking-wider mb-0.5">TOTAL DUE</span>
                          <span className="text-base font-bold text-slate-900">$ {fmt(amountDue.totalBalanceWithProfit)}</span>
                        </div>
                      </td>
                      <td rowSpan={4} className="px-3 py-2 text-center align-middle">
                        <div className="inline-flex flex-col items-center gap-1.5">
                          <div className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-1.5 text-[10px] text-slate-400 font-medium whitespace-nowrap">
                            Product & Shipping Cost Only
                          </div>
                          <div className={`text-sm font-bold ${amountDue.totalCostOnlyDue < 0 ? 'text-emerald-600' : 'text-red-600'}`}>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
        ...
        <div className="bg-slate-800 px-4 py-2.5 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xs font-semibold text-white">Amazon Product Revenue Summary</h3>
            <span className="text-[10px] bg-white/20 text-white px-2 py-0.5 rounded-full font-medium">{summary.length} products</span>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                <th className="text-left px-3 py-2 font-semibold text-slate-500 text-[10px] uppercase tracking-wider sticky left-0 bg-slate-50 z-10 min-w-[180px] whitespace-nowrap">Product Name</th>
                <th className="text-right px-2.5 py-2 font-semibold text-slate-500 text-[10px] uppercase tracking-wider whitespace-nowrap">Qty Sold</th>
                <th className="text-right px-2.5 py-2 font-semibold text-slate-500 text-[10px] uppercase tracking-wider whitespace-nowrap">Amazon Product Rev</th>
                <th className="text-right px-2.5 py-2 font-semibold text-slate-500 text-[10px] uppercase tracking-wider whitespace-nowrap">Amazon Shipping Rev</th>
                <th className="text-right px-2.5 py-2 font-semibold text-slate-500 text-[10px] uppercase tracking-wider whitespace-nowrap">Total Amazon Rev</th>
                <th className="text-right px-2.5 py-2 font-semibold text-slate-500 text-[10px] uppercase tracking-wider whitespace-nowrap">Product Cost</th>
                <th className="text-right px-2.5 py-2 font-semibold text-slate-500 text-[10px] uppercase tracking-wider whitespace-nowrap">Shipping Cost</th>
                <th className="text-right px-2.5 py-2 font-semibold text-slate-500 text-[10px] uppercase tracking-wider whitespace-nowrap">Total Cost</th>
                <th className="text-right px-2.5 py-2 font-semibold text-slate-500 text-[10px] uppercase tracking-wider whitespace-nowrap">Product Profit</th>
                <th className="text-right px-2.5 py-2 font-semibold text-slate-500 text-[10px] uppercase tracking-wider whitespace-nowrap">Shipping Profit</th>
                <th className="text-right px-2.5 py-2 font-semibold text-slate-500 text-[10px] uppercase tracking-wider whitespace-nowrap">Total Profit</th>
                <th className="text-right px-2.5 py-2 font-semibold text-slate-500 text-[10px] uppercase tracking-wider whitespace-nowrap">IPF Profit</th>
                <th className="text-right px-2.5 py-2 font-semibold text-slate-500 text-[10px] uppercase tracking-wider whitespace-nowrap">AS Profit</th>
              </tr>
            </thead>
            <tbody>
              {summary.map((p, idx) => (
                <tr key={p.productName}
                  ...
                  <td className="px-3 py-1.5 font-medium text-slate-800 sticky left-0 z-10 whitespace-nowrap text-[11px]" style={{ backgroundColor: idx % 2 === 0 ? '#fff' : '#f8fafc' }}>{p.productName}</td>
                  <td className="px-2.5 py-1.5 text-right font-mono text-slate-600 whitespace-nowrap text-[11px]">{fmtInt(p.quantitySold)}</td>
                  <td className="px-2.5 py-1.5 text-right font-mono text-slate-600 whitespace-nowrap text-[11px]">$ {fmt(p.amazonProductRevenue)}</td>
                  <td className="px-2.5 py-1.5 text-right font-mono text-slate-600 whitespace-nowrap text-[11px]">$ {fmt(p.amazonShippingRevenue)}</td>
                  <td className="px-2.5 py-1.5 text-right font-mono font-medium text-slate-800 whitespace-nowrap text-[11px]">$ {fmt(p.totalAmazonRevenue)}</td>
                  <td className="px-2.5 py-1.5 text-right font-mono text-slate-600 whitespace-nowrap text-[11px]">$ {fmt(p.productCost)}</td>
                  <td className="px-2.5 py-1.5 text-right font-mono text-slate-600 whitespace-nowrap text-[11px]">$ {fmt(p.shippingCost)}</td>
                  <td className="px-2.5 py-1.5 text-right font-mono font-medium text-slate-800 whitespace-nowrap text-[11px]">$ {fmt(p.totalCost)}</td>
                  <td className={`px-2.5 py-1.5 text-right font-mono whitespace-nowrap text-[11px] ${p.productProfit >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                    {p.productProfit < 0 ? `(${fmt(Math.abs(p.productProfit))})` : `$ ${fmt(p.productProfit)}`}
                  </td>
                  <td className={`px-2.5 py-1.5 text-right font-mono whitespace-nowrap text-[11px] ${p.shippingProfit >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                    {p.shippingProfit < 0 ? `(${fmt(Math.abs(p.shippingProfit))})` : `$ ${fmt(p.shippingProfit)}`}
                  </td>
                  <td className={`px-2.5 py-1.5 text-right font-mono font-medium whitespace-nowrap text-[11px] ${p.totalProfit >= 0 ? 'text-emerald-700' : 'text-red-700'}`}>
                    $ {fmt(p.totalProfit)}
                  </td>
                  <td className="px-2.5 py-1.5 text-right font-mono text-slate-700 whitespace-nowrap text-[11px]">$ {fmt(p.ipfProfit)}</td>
                  <td className="px-2.5 py-1.5 text-right font-mono text-slate-700 whitespace-nowrap text-[11px]">$ {fmt(p.activateSwagProfit)}</td>
                </tr>
              ))}
            </tbody>
            {/* Totals row */}
            <tfoot>
              <tr className="bg-slate-800 text-white font-medium text-[11px]">
                <td className="px-3 py-2 sticky left-0 z-10 font-semibold uppercase text-[10px] tracking-wider whitespace-nowrap" style={{ backgroundColor: '#1e293b' }}>Totals</td>
                <td className="px-2.5 py-2 text-right font-mono whitespace-nowrap">{fmtInt(totals.quantitySold)}</td>
                <td className="px-2.5 py-2 text-right font-mono whitespace-nowrap">$ {fmt(totals.amazonProductRevenue)}</td>
                <td className="px-2.5 py-2 text-right font-mono whitespace-nowrap">$ {fmt(totals.amazonShippingRevenue)}</td>
                <td className="px-2.5 py-2 text-right font-mono whitespace-nowrap">$ {fmt(totals.totalAmazonRevenue)}</td>
                <td className="px-2.5 py-2 text-right font-mono whitespace-nowrap">$ {fmt(totals.productCost)}</td>
                <td className="px-2.5 py-2 text-right font-mono whitespace-nowrap">$ {fmt(totals.shippingCost)}</td>
                <td className="px-2.5 py-2 text-right font-mono whitespace-nowrap">$ {fmt(totals.totalCost)}</td>
                <td className="px-2.5 py-2 text-right font-mono whitespace-nowrap">$ {fmt(totals.productProfit)}</td>
                <td className="px-2.5 py-2 text-right font-mono whitespace-nowrap">$ {fmt(totals.shippingProfit)}</td>
                <td className="px-2.5 py-2 text-right font-mono whitespace-nowrap">$ {fmt(totals.totalProfit)}</td>
                <td className="px-2.5 py-2 text-right font-mono whitespace-nowrap">$ {fmt(totals.ipfProfit)}</td>
                <td className="px-2.5 py-2 text-right font-mono whitespace-nowrap">$ {fmt(totals.activateSwagProfit)}</td>
              </tr>
            </tfoot>
          </table>
        </div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-xs font-semibold text-slate-800">Upcoming Payouts</h3>
          <span className="text-[10px] text-slate-400">Profit due 90 days after delivery</span>
        </div>
            ...
            return (
              <div key={bucket.label}
                ...
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-[10px] font-semibold uppercase tracking-wider text-slate-500">{bucket.label}</span>
                ...
                <p className="text-base font-bold text-slate-900">${fmt(total)}</p>
              </div>
            );
          })}
        </div>
    ...
    <div className="space-y-4">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
        ...
        <div className="flex items-start gap-2">
          <div
            className="w-7 h-7 rounded-lg bg-slate-800 flex items-center justify-center shrink-0">
            <Sparkles className="w-3.5 h-3.5 text-white" />
          </div>
          <div className="flex-1">
            <h3 className="text-xs font-semibold text-slate-800 mb-1">AI Collection Insights</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
              {kpis.pastDueCount > 0 && (
                <div className="flex items-start gap-1.5 bg-white rounded-lg p-2.5 border border-slate-200">
                  <AlertTriangle className="w-3.5 h-3.5 text-red-500 mt-0.5 shrink-0" />
                  <p className="text-[11px] text-slate-700"><strong>{kpis.pastDueCount} invoices</strong> are past due totaling <strong>${fmt(kpis.pastDueAmount)}</strong>. Consider sending collection statements.</p>
                </div>
              )}
              <div className="flex items-start gap-1.5 bg-white rounded-lg p-2.5 border border-slate-200">
                <Zap className="w-3.5 h-3.5 text-amber-500 mt-0.5 shrink-0" />
                <p className="text-[11px] text-slate-700">Collection rate is at <strong>{kpis.collectionRate.toFixed(1)}%</strong>. {kpis.collectionRate > 50 ? 'Good progress!' : 'Needs attention.'}</p>
              </div>
              <div className="flex items-start gap-1.5 bg-white rounded-lg p-2.5 border border-slate-200">
                <Mail className="w-3.5 h-3.5 text-blue-500 mt-0.5 shrink-0" />
                <p className="text-[11px] text-slate-700">Generate statements for your sales reps to accelerate collections across all accounts.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.25 }}
        ...
        <div className="relative flex-1 min-w-[200px] max-w-md">
          ...
          <input value={searchQuery} onChange={e => { setSearchQuery(e.target.value); setPage(1); }}
            ...
            className="w-full pl-9 pr-4 py-2 text-xs bg-white border border-slate-200 rounded-lg focus:ring-2 focus:ring-slate-200 focus:border-slate-400 outline-none transition-all" />
        </div>
        ...
        <button onClick={() => setShowStatementModal(true)}
          className="px-3 py-2 text-xs font-semibold bg-slate-800 text-white rounded-lg hover:bg-slate-700 transition-all flex items-center gap-1.5">
          <Send className="w-3.5 h-3.5" /> Generate Statement
        </button>
      </motion.div>
              ...
              <tr className="bg-slate-50 border-b border-slate-200">
                {[
                  ...
                  <th key={col.key} onClick={() => handleSort(col.key)}
                    className="px-3 py-2 text-left text-[10px] font-semibold text-slate-500 uppercase tracking-wider cursor-pointer hover:text-slate-900 select-none whitespace-nowrap">
                ...
                <th className="px-3 py-2 text-[10px] font-semibold text-slate-500 uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((inv: CorporateInvoice, idx: number) => {
                ...
                return (
                  <motion.tr key={inv.id}
                    ...
                    <td className="px-3 py-1.5 font-mono font-medium text-slate-700 whitespace-nowrap">{inv.invoiceNumber}</td>
                    <td className="px-3 py-1.5 whitespace-nowrap">
                       <div className="flex items-center gap-1.5 min-w-0">
                         {customerLogoMap[(inv.customer || '').trim().toLowerCase()] ? (
                           <div className="w-6 h-5 rounded flex items-center justify-center overflow-hidden border border-slate-200 bg-white shrink-0">
                             <img
                           ...
                           <div className="w-6 h-5 rounded bg-gradient-to-br from-slate-600 to-slate-800 flex items-center justify-center shrink-0">
                             <span className="text-[10px] font-bold text-white">
                               {(inv.customer || '?').charAt(0).toUpperCase()}
                             </span>
                           </div>
                         )}
                         <span className="font-medium text-slate-800 truncate">{inv.customer}</span>
                       </div>
                     </td>
                    <td className="px-3 py-1.5 text-slate-600 whitespace-nowrap">{inv.salesRep}</td>
                    <td className="px-3 py-1.5 text-slate-500 whitespace-nowrap">{inv.items}</td>
                    <td className="px-3 py-1.5 font-mono font-medium text-slate-800 whitespace-nowrap">${fmt(inv.amount)}</td>
                    <td className="px-3 py-1.5 font-mono text-emerald-600 whitespace-nowrap">${fmt(inv.amountPaid)}</td>
                    <td className="px-3 py-1.5 text-slate-500 whitespace-nowrap">{inv.issueDate}</td>
                    <td className="px-3 py-1.5 whitespace-nowrap">
                      <span className={overdueDays > 0 ? 'text-red-600 font-medium' : 'text-slate-500'}>
                        {inv.dueDate}
                        {overdueDays > 0 && <span className="ml-1 text-[10px]">({overdueDays}d late)</span>}
                      </span>
                    </td>
                    <td className="px-3 py-1.5 whitespace-nowrap">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold border ${getStatusColor(inv.status)}`}>
                    ...
                    <td className="px-3 py-1.5">
                      <div className="flex items-center gap-0.5">
                        <button className="p-1 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded transition-all" title="View">
                          <Eye className="w-3 h-3" />
                        </button>
                        <button className="p-1 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded transition-all" title="Send Reminder">
                          <Mail className="w-3 h-3" />
                        </button>
                        <button className="p-1 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded transition-all" title="Download">
      ...
      <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }}
        ...
        <div className="bg-slate-800 px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xs font-semibold text-white">Generate Collection Statement</h2>
          </div>
        ...
        <div className="p-4 space-y-3">
          <div>
            <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-1 block">Sales Rep / Account Manager</label>
            <select value={selectedRep} onChange={e => setSelectedRep(e.target.value)}
              className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs focus:ring-2 focus:ring-slate-200 focus:border-slate-400 outline-none">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-2.5 text-center">
              <p className="text-[9px] font-semibold text-slate-400 uppercase">Outstanding</p>
              <p className="text-base font-bold text-slate-800">${fmt(totalOwed)}</p>
            </div>
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-2.5 text-center">
              <p className="text-[9px] font-semibold text-slate-400 uppercase">Invoices</p>
              <p className="text-base font-bold text-slate-800">{repInvoices.length}</p>
            </div>
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-2.5 text-center">
              <p className="text-[9px] font-semibold text-slate-400 uppercase">Past Due</p>
              <p className="text-base font-bold text-slate-800">{pastDueCount}</p>
            </div>
          </div>
                ...
                <tbody>
                  {repInvoices.map(inv => (
                    <tr key={inv.id} className="border-t border-slate-100 hover:bg-slate-50">
                      <td className="px-3 py-2 font-mono font-medium text-slate-700">{inv.invoiceNumber}</td>
                      <td className="px-3 py-2 text-slate-700">{inv.customer}</td>
                      <td className="px-3 py-2 text-right font-mono font-medium text-slate-800">${fmt(inv.amount - inv.amountPaid)}</td>
                      <td className="px-3 py-2 text-slate-500">{inv.dueDate}</td>
                      <td className="px-3 py-2">
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold border ${getStatusColor(inv.status)}`}>
                          {getStatusLabel(inv.status)}
                        </span>
                      </td>
                    </tr>
          ...
          <div className="flex justify-end gap-3 pt-2">
            <button onClick={onClose}
              className="px-3 py-2 text-xs font-medium text-slate-600 bg-slate-100 rounded-lg hover:bg-slate-200 transition-all">
              Cancel
            </button>
            <button onClick={() => { toast.success('Statement generated and ready to send!'); onClose(); }}
              className="px-4 py-2 text-xs font-semibold bg-slate-800 text-white rounded-lg hover:bg-slate-700 transition-all flex items-center gap-1.5">
              <Printer className="w-3.5 h-3.5" /> Generate & Send
            </button>
          </div>
        </div>
      </motion.div>
