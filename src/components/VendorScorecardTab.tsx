...
function MetricBar({ metric, onUpdate }: { metric: ScorecardMetric; onUpdate: (m: ScorecardMetric) => void }) {
  const [editing, setEditing] = useState(false);
  const [editScore, setEditScore] = useState(metric.score);
  const color = metric.score >= 85 ? 'bg-emerald-500' : metric.score >= 70 ? 'bg-blue-500' : metric.score >= 50 ? 'bg-amber-500' : 'bg-red-500';
  const textColor = metric.score >= 85 ? 'text-emerald-600' : metric.score >= 70 ? 'text-blue-600' : metric.score >= 50 ? 'text-amber-600' : 'text-red-600';
    ...
    <div className="flex items-center gap-3 py-2 group">
      ...
      {editing ? (
        <div className="flex items-center gap-1 shrink-0">
          <input type="number" min={0} max={100} value={editScore} onChange={e => setEditScore(Number(e.target.value))} className="w-14 px-1.5 py-0.5 border border-slate-300 rounded text-xs text-center focus:outline-none focus:border-blue-500" />
          <button onClick={() => { onUpdate({ ...metric, score: editScore }); setEditing(false); }} className="p-0.5 text-emerald-600 hover:bg-emerald-50 rounded"><Save className="w-3.5 h-3.5" /></button>
...
export function VendorScorecardTab({ vendorId, vendorName, purchaseOrders }: { vendorId: string; vendorName: string; purchaseOrders: any[] }) {
  ...
  const saveScorecard = async (_card: VendorScorecard) => {
    setSaving(true);
    setSaving(false);
  };
              ...
              {categories.map(cat => (
                <div key={cat} className="mb-5 last:mb-0">
                  ...
                  {scorecard.metrics.filter(m => m.category === cat).map((m) => {
                    ...
                    return <MetricBar key={m.label} metric={m} onUpdate={(updated) => handleMetricUpdate(globalIdx, updated)} />;
                  })}
                </div>
              ))}
            </div>
