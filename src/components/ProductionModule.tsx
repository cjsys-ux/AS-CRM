...
function AddProductionOrderDrawer({
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    toast.success(editOrder ? 'Production order updated' : 'Production order created');
    onSuccess();
    onClose();
    setIsSaving(false);
  };
              ...
              <div className="bg-slate-50 rounded-2xl p-4 border border-slate-200">
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <InputField label="Quality %">
                    <input type="number" min="0" max="100" value={form.quality || ''} onChange={e => setForm(p => ({ ...p, quality: parseInt(e.target.value) || 0 }))} className={inputCls} />
                  </InputField>
                </div>
              </div>
...
export function ProductionModule() {
  ...
  const handleDeleteOrder = async () => {
    ...
    setOrders(prev => prev.filter(item => item.id !== orderToDelete.id));
