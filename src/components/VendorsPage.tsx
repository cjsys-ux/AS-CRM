            ...
            <div className="max-w-[1800px] mx-auto">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h1 className="text-3xl font-bold text-slate-900 mb-1">Vendor Database</h1>
                    <p className="text-slate-500">Manage your vendor relationships and contacts</p>
                  </div>
                </div>
                <motion.button
                ...
                <div className="flex items-center gap-4">
                  <div className="flex-1 relative">
                    ...
                    <input
                      ...
                      className="w-full pl-12 pr-4 py-3 bg-slate-50 border-2 border-slate-200 rounded-xl text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all"
                    />
                  </div>
                  <motion.button
                      ...
                      <table className="w-full">
                        <thead>
                          <tr className="bg-slate-50 border-b border-slate-200">
                            {isColVisible('vendorName') && <th className="text-left px-4 py-4 text-xs font-bold text-slate-600 uppercase tracking-wider whitespace-nowrap">Vendor Name</th>}
                            {isColVisible('status') && <th className="text-left px-4 py-4 text-xs font-bold text-slate-600 uppercase tracking-wider whitespace-nowrap">Status</th>}
                            {isColVisible('type') && <th className="text-left px-4 py-4 text-xs font-bold text-slate-600 uppercase tracking-wider whitespace-nowrap">Type</th>}
                            {isColVisible('contact') && <th className="text-left px-4 py-4 text-xs font-bold text-slate-600 uppercase tracking-wider whitespace-nowrap">Contact</th>}
                            {isColVisible('location') && <th className="text-left px-4 py-4 text-xs font-bold text-slate-600 uppercase tracking-wider whitespace-nowrap">Location</th>}
                            {isColVisible('paymentTerms') && <th className="text-left px-4 py-4 text-xs font-bold text-slate-600 uppercase tracking-wider whitespace-nowrap">Payment Terms</th>}
                            {isColVisible('actions') && <th className="text-left px-4 py-4 text-xs font-bold text-slate-600 uppercase tracking-wider whitespace-nowrap">Actions</th>}
                          </tr>
                        </thead>
                        <tbody>
                          {paginatedVendors.map((vendor, idx) => (
                            ...
                            >
                              {isColVisible('vendorName') && <td className="px-4 py-4">
                                <div className="flex items-center gap-3">
                                  {vendor.logo ? (
                                    ...
                                    <div className="w-10 h-10 bg-gradient-to-br from-purple-500 to-indigo-600 rounded-xl flex items-center justify-center shadow-md shrink-0">
                                      <span className="text-white text-sm font-bold">{getVendorInitials(vendor.name)}</span>
                                    </div>
                                  )}
                                  <div>
                                    <div className="font-semibold text-slate-900">{vendor.name}</div>
                                    {vendor.email && <div className="text-xs text-slate-400">{vendor.email}</div>}
                                  </div>
                                </div>
                              </td>}
                              {isColVisible('status') && <td className="px-4 py-4 whitespace-nowrap">
                                <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold border ${getStatusColor(vendor.status)}`}>
                                  {vendor.status || 'Active'}
                                </span>
                              </td>}
                              {isColVisible('type') && <td className="px-4 py-4 whitespace-nowrap">
                                <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold border ${getTypeColor(vendor.type)}`}>
                                  {vendor.type || '—'}
                                </span>
                              </td>}
                              {isColVisible('contact') && <td className="px-4 py-4 whitespace-nowrap">
                                <span className="text-sm text-slate-700">{vendor.contactName || vendor.contact || '—'}</span>
                              </td>}
                              {isColVisible('location') && <td className="px-4 py-4 whitespace-nowrap">
                              ...
                              {isColVisible('paymentTerms') && <td className="px-4 py-4 whitespace-nowrap">
                                <span className="text-sm text-slate-700">{vendor.paymentTerms || vendor.netTerms || '—'}</span>
                              </td>}
