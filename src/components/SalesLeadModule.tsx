...
import {
  ...
  BarChart3, AlertTriangle, CheckCircle2, Sparkles, XCircle, ChevronUp, Loader2
} from 'lucide-react';
...
const headers_json = { 'Content-Type': 'application/json', 'Authorization': `Bearer ${publicAnonKey}` };
...
function LeadDrawer({ isOpen, onClose, onSave, lead }: { isOpen: boolean; onClose: () => void; onSave: (data: any) => void; lead?: SalesLead | null }) {
  const [form, setForm] = useState({
    title: '', company: '', companyId: '', contactName: '', contactEmail: '', contactPhone: '',
    amount: '', stage: 'lead-received', source: 'Website', dealSize: 'Small',
  ...
  useEffect(() => {
    const h = (e: MouseEvent) => { if (custRef.current && !custRef.current.contains(e.target as Node)) setShowCustomerList(false); };
    document.addEventListener('mousedown', h); return () => document.removeEventListener('mousedown', h);
  }, []);
  ...
  useEffect(() => {
    if (lead) {
      setForm({
        ...
        amount: String(lead.amount), stage: lead.stage, source: lead.source,
        dealSize: lead.dealSize, productType: lead.productType,
      ...
      setForm({ title: '', company: '', companyId: '', contactName: '', contactEmail: '', contactPhone: '', amount: '', stage: 'lead-received', source: 'Website', dealSize: 'Small', productType: 'Apparel', inHandsDate: '', owner: '', notes: '', quantity: '' });
  ...
  const fetchCustomers = useCallback(async () => {
    try {
      ...
      const res = await fetch(`${API_URL}/customers`, { headers: headers_json });
  ...
  const selectCustomer = (cust: Customer) => {
    setForm({
      ...
      company: cust.name,
      companyId: cust.id,
      contactName: cust.contactName || cust.contact || '',
  ...
  const handleSubmit = () => {
    ...
    onSave({
      ...
      tags: form.productType ? [form.productType] : [],
      ...(lead ? { id: lead.id } : {}),
    });
  };
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 30, stiffness: 300 }} className="fixed right-0 top-0 bottom-0 w-[580px] bg-white shadow-2xl z-50 flex flex-col">
            {/* Header */}
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 mb-2">
                      <button
                        type="button"
                        onClick={() => { setIsExistingCompany(false); setForm({ ...form, companyId: '' }); }}
                        className={`flex-1 px-3 py-2 rounded-lg text-xs font-semibold border-2 transition-all ${
                          !isExistingCompany ? 'bg-indigo-50 border-indigo-300 text-indigo-700' : 'bg-white border-slate-200 text-slate-500 hover:border-slate-300'
                        }`}
                      >
                      ...
                      <div className="relative" ref={custRef}>
                        ...
                        {form.companyId && (
                          <div className="mt-1.5 flex items-center gap-2 px-3 py-1.5 bg-indigo-50 border border-indigo-200 rounded-lg">
                            ...
                            <button onClick={() => { setForm({ ...form, company: '', companyId: '', contactName: '', contactEmail: '', contactPhone: '' }); setCustomerSearch(''); }} className="ml-auto p-0.5 hover:bg-indigo-100 rounded">
                        ...
                        <AnimatePresence>
                          {showCustomerList && customerSearch && (
                            <motion.div
                              ...
                              className="absolute top-full left-0 right-0 mt-1 bg-white rounded-xl border border-slate-200 shadow-2xl z-50 max-h-48 overflow-y-auto"
                            >
                              {filteredCustomers.length === 0 ? (
                                <div className="px-4 py-6 text-center">
                                  ...
                                  <button
                                    type="button"
                                    onClick={() => { setIsExistingCompany(false); setForm({ ...form, company: customerSearch }); setShowCustomerList(false); }}
                                ...
                                >
                                  ...
                                  <div className="min-w-0">
                                    ...
                                    <div className="text-[11px] text-slate-500 truncate">{cust.contactName || cust.contact || cust.email || ''}</div>
                                  </div>
                                  {cust.status && (
                                    <span className={`ml-auto text-[10px] font-bold px-1.5 py-0.5 rounded ${cust.status === 'Active' ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>{cust.status}</span>
                                  )}
                                </button>
                              ))}
                            </motion.div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="number" value={form.amount} onChange={e => setForm({ ...form, amount: e.target.value })} placeholder="0.00" className={inputCls} />
                    </div>
                    <CustomDropdown
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <CustomDropdown
                      label="Deal Size"
                      value={form.dealSize}
                      options={DEAL_SIZES.map(s => ({ value: s, label: s }))}
                      onChange={v => setForm({ ...form, dealSize: v })}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input value={form.owner} onChange={e => setForm({ ...form, owner: e.target.value })} placeholder="John Smith" className={inputCls} />
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input value={form.contactName} onChange={e => setForm({ ...form, contactName: e.target.value })} placeholder="Full name" className={inputCls} />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className={labelCls}>Email</label>
                      <input type="email" value={form.contactEmail} onChange={e => setForm({ ...form, contactEmail: e.target.value })} placeholder="email@company.com" className={inputCls} />
                    </div>
                    <div>
                      <label className={labelCls}>Phone</label>
                      <input value={form.contactPhone} onChange={e => setForm({ ...form, contactPhone: e.target.value })} placeholder="(555) 123-4567" className={inputCls} />
                    </div>
                  </div>
                </div>
              </div>
...
// ────── Deal Card (Draggable) ──────
function DealCard({ lead, stage, onEdit, onDelete, onMove, onDragStart }: {
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-xl border border-slate-200/80 shadow-sm hover:shadow-lg hover:border-slate-300 transition-all group cursor-grab active:cursor-grabbing active:shadow-xl active:scale-[1.02] relative overflow-hidden"
    >
      {/* Color accent line */}
      ...
      <div className="p-3.5">
            ...
            <AnimatePresence>
              {menuOpen && (
                ...
                >
                  ...
                  {PIPELINE_STAGES.filter(s => s.id !== lead.stage && s.id !== 'closed-lost').map(s => (
        ...
        <div className="flex flex-wrap items-center gap-1.5 mb-2.5">
          <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold ${stage.bg} ${stage.text} border ${stage.border}`}>{lead.dealSize}</span>
...
export function SalesLeadModule() {
  ...
  const handleSave = async (data: any) => {
    try {
      ...
      const res = await fetch(url, { method: isEdit ? 'PUT' : 'POST', headers: headers_json, body: JSON.stringify(data) });
  ...
  return (
    <>
      <div className="flex-1 flex flex-col bg-gradient-to-br from-slate-50 via-slate-50 to-indigo-50/30 overflow-hidden">
        {/* Header */}
        <div className="bg-white border-b border-slate-200 px-8 py-6">
          <div className="max-w-[2200px] mx-auto">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2.5">
                <button onClick={() => setViewMode(viewMode === 'board' ? 'list' : 'board')} className="px-4 py-2.5 bg-white text-slate-700 text-sm font-semibold rounded-xl hover:bg-slate-50 transition-colors border border-slate-200">
        ...
        <div className="px-8 mb-4 shrink-0">
          <div className="max-w-[2200px] mx-auto">
            <div className="flex items-center gap-2.5">
              ...
              <select value={sourceFilter} onChange={e => setSourceFilter(e.target.value)} className="px-3.5 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-medium text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 shadow-sm cursor-pointer">
                <option>All Sources</option>
                {LEAD_SOURCES.map(s => <option key={s}>{s}</option>)}
              </select>
              <select value={ownerFilter} onChange={e => setOwnerFilter(e.target.value)} className="px-3.5 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-medium text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 shadow-sm cursor-pointer">
            ...
            <div className="flex gap-3 h-full" style={{ minWidth: `${visibleStages.length * 260}px` }}>
              {visibleStages.map((stage) => {
                ...
                return (
                  <div
                    key={stage.id}
                    className={`flex-1 min-w-[240px] max-w-[300px] flex flex-col rounded-2xl transition-all ${
