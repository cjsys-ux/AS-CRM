...
export function ContactsModule() {
  ...
  const handleSaveContact = async (contactData: any) => {
    ...
    if (isEdit) {
      setContactsData(prev => prev.map(item => item.id === contactData.id ? { ...item, ...contactData } : item));
    } else {
      const newItem = { ...contactData, id: Date.now().toString() };
      setContactsData(prev => [...prev, newItem]);
    }
  ...
  const handleDeleteContact = async () => {
    ...
    setContactsData(prev => prev.filter(item => item.id !== contactToDelete.id));
        ...
        <div className="max-w-[1800px] mx-auto">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-slate-900 mb-1">Contacts</h1>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    {isColVisible('name') && <th className="text-left px-4 py-4 text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">
                    ...
                    {isColVisible('company') && <th className="text-left px-4 py-4 text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">
                    ...
                    {isColVisible('position') && <th className="text-left px-4 py-4 text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">
                    ...
                    {isColVisible('contactInfo') && <th className="text-left px-4 py-4 text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">Contact Info</th>}
                    {isColVisible('type') && <th className="text-left px-4 py-4 text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">
                    ...
                    {isColVisible('status') && <th className="text-left px-4 py-4 text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">
                    ...
                    {isColVisible('owner') && <th className="text-left px-4 py-4 text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">
                    ...
                    {isColVisible('createdAt') && <th className="text-left px-4 py-4 text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">
                    ...
                    {isColVisible('lastContact') && <th className="text-left px-4 py-4 text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">
                ...
                <tbody className="divide-y divide-slate-100">
                  {paginatedContacts.map((contact, index) => (
                    ...
                    >
                      {isColVisible('name') && <td className="px-4 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="font-semibold text-slate-900 whitespace-nowrap">{contact.name}</div>
                        </div>
                      </td>}
                      {isColVisible('company') && <td className="px-4 py-4">
                        <div className="font-medium text-slate-900 whitespace-nowrap">{contact.company}</div>
                        <div className="text-xs text-slate-400 mt-0.5 whitespace-nowrap">{contact.country}</div>
                      </td>}
                      {isColVisible('position') && <td className="px-4 py-4">
                        <div className="text-sm text-slate-700">{contact.position}</div>
                      </td>}
                      {isColVisible('contactInfo') && <td className="px-4 py-4">
                      ...
                      {isColVisible('owner') && <td className="px-4 py-4">
                        {contact.owner ? (
                          <div className="text-sm font-medium text-slate-700 whitespace-nowrap">{contact.owner}</div>
                        ) : (
                          <span className="text-xs text-slate-400 italic">No Owner</span>
                        )}
                      </td>}
                      {isColVisible('createdAt') && <td className="px-4 py-4 whitespace-nowrap">
                        <span className="text-sm text-slate-600">{contact.createdAt ? (() => {
                          const d = new Date(contact.createdAt);
                          if (isNaN(d.getTime())) return contact.createdAt;
                          return `${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}-${d.getFullYear()}`;
                        })() : '—'}</span>
                      </td>}
                      {isColVisible('lastContact') && <td className="px-4 py-4 whitespace-nowrap">
                        <span className="text-sm text-slate-600">{contact.lastContact ? (() => {
