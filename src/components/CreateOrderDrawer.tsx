...
export function CreateOrderDrawer({ isOpen, onClose, onCreated }: CreateOrderDrawerProps) {
  ...
  useEffect(() => {
    ...
    const fetchCustomers = async () => {
      ...
      try {
        ...
        const res = await fetch('/api/customers/list', { signal: controller.signal });
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      ...
      try {
        ...
        const response = await fetch('/api/products/list', { signal: controller.signal });
        clearTimeout(timeoutId);
        ...
        if (response.ok) {
          ...
          if (rows.length > 0) {
            const formattedProducts: Product[] = rows.map((p: any) => ({
              id: p.id || p.sku,
              name: p.name,
              sku: p.sku,
              category: p.category,
              price: p.price ?? (typeof p.basePrice === 'number' ? `$${p.basePrice}` : undefined),
  ...
  const fetchCustomerContactsAndAddresses = async (customerId: string) => {
    try {
      const [custRes, contactsRes, addressesRes] = await Promise.all([
      ...
      if (custRes.ok) {
        ...
        if (customer?.contacts) setSavedContacts(customer.contacts);
        if (customer?.addresses) setSavedAddresses(customer.addresses);
      }
    } catch (error) {
