        ...
        if (removeBackground) {
          try {
            ...
            if (data.success && data.imageUrl) {
              ...
              console.error('Background removal failed, using original:', data.error);
              toast.warning('Background removal unavailable — using original image', {
                description: data.error || 'The service could not process this image.',
              });
            }
          } catch (err) {
            console.error('Background removal error, using original:', err);
            toast.warning('Background removal unavailable — using original image');
          }
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (vendorData?.id) {
        ...
        else console.error('Error updating vendor:', data.error);
      } else {
        ...
        if (data.success) {
          ...
          setUploadedLogo(null);
        } else console.error('Error creating vendor:', data.error);
      }
    } catch (error) {
