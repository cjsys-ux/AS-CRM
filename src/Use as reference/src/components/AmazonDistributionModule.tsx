...
function AddOrderDrawer({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/amazon-orders`, {
        method: 'POST',
        headers: {
...
function EditOrderDrawer({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/amazon-orders/${order.id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${publicAnonKey}`,
          'Content-Type': 'application/json',
...
export function AmazonDistributionModule() {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      ...
      if (data.success) {
        ...
        setAvailableProducts([...dbParsed, ...liveParsed]);
      }
    } catch (error) {
  ...
  const handleDeleteOrder = async () => {
    ...
    setIsDeleting(true);
    try {
      const response = await fetch(`${API_URL}/amazon-orders/${orderToDelete.id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${publicAnonKey}` },
      });
      const data = await response.json();
  ...
  const handleTogglePaid = async (order: AmazonOrder) => {
    try {
      const response = await fetch(`${API_URL}/amazon-orders/${order.id}`, {
        ...
        body: JSON.stringify({ amazonPaid: !order.amazonPaid }),
      });
      const data = await response.json();
      if (data.success) {
  ...
  const handleBulkSave = async () => {
    ...
    await Promise.all([...selectedIds].map(async (id) => {
      try {
        ...
        const res = await fetch(`${API_URL}/amazon-orders/${id}`, {
          method: 'PUT',
