...
export function VendorDetailView({ vendor, onBack, onDelete, onVendorUpdated }: VendorDetailViewProps) {
  ...
  const fetchVendor = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/vendors/${vendor.id}`, {
        headers: { 'Authorization': `Bearer ${publicAnonKey}` },
      });
      const data = await res.json();
      if (data.success && data.vendor) {
        setVendorData(data.vendor);
        setAddresses(data.vendor.addresses || []);
      }
    } catch (error) {
  ...
  const saveVendorField = async (updates: Record<string, any>) => {
    ...
    try {
      ...
      if (data.success && data.vendor) {
        setVendorData(data.vendor);
        if (data.vendor.addresses) setAddresses(data.vendor.addresses);
  ...
  const confirmDeleteAddress = async () => {
    ...
    try {
      await fetch(`${API_URL}/vendors/${vendor.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${publicAnonKey}`,
