...
export function InventoryModule() {
  ...
  useEffect(() => {
    fetchItems();
    // Fetch customer logos for table display
    (async () => {
      try {
        const res = await fetch(`${API_URL}/customers`, { headers: { 'Authorization': `Bearer ${publicAnonKey}` } });
        const data = await res.json();
        if (data.success) {
          const map: Record<string, string> = {};
          (data.customers || []).forEach((c: any) => { if (c.name && c.logo) map[c.name.trim().toLowerCase()] = c.logo; });
          setCustomerLogoMap(map);
        }
      } catch (e) { console.log('Failed to fetch customer logos:', e); }
    })();
  }, []);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/inventory/${deleteItem.id}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${publicAnonKey}`,
  ...
  const handleDiscontinueConfirm = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/inventory/${discontinueItem.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${publicAnonKey}`,
      ...
      if (data.success) {
        ...
        console.error('Error discontinuing item:', data.error);
        toast.error('Failed to discontinue item');
      }
    } catch (error) {
      console.error('Error discontinuing item:', error);
      toast.error('Error discontinuing item');
    } finally {
  ...
  const handleReactivateItem = async (item: InventoryItem) => {
    try {
      ...
      if (data.success) {
        ...
        console.error('Error reactivating item:', data.error);
