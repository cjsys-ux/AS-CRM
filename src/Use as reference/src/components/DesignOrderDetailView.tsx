...
export function DesignOrderDetailView({ task: initialTask, onBack, onTaskUpdated }: DesignOrderDetailViewProps) {
  ...
  useEffect(() => {
    const loadImprintLocations = async () => {
      ...
      try {
        ...
        console.log('[Imprint Locations] API response success:', data.success);
        
        if (!data.success || !data.products) {
          console.log('[Imprint Locations] No products data, using defaults');
  ...
  useEffect(() => {
    const loadImprintMethods = async () => {
      ...
      try {
        // Fetch products from Supabase KV API instead of localStorage
        const response = await fetch(`${API_URL}/productdb`, {
          headers: { 'Authorization': `Bearer ${publicAnonKey}` },
        });
        const data = await response.json();
        ...
        if (!data.success || !data.products) {
          console.log('[Imprint Methods] No products data from API');
  ...
  const updateTask = async (updates: Partial<DesignTask>) => {
    setSaving(true);
    try {
      const res = await fetch(`${API_URL}/design-tasks/${task.id}`, {
        method: 'PUT',
  ...
  const uploadFile = async (type: 'art' | 'mockup', base64Data: string, fileName: string): Promise<string | null> => {
    try {
      const res = await fetch(`${API_URL}/design-tasks/${task.id}/upload`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${publicAnonKey}` },
        body: JSON.stringify({ type, base64Data, fileName }),
      });
      const data = await res.json();
      if (data.success) {
        setTask(data.task);
