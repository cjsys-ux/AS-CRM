...
export function DesignLabModule() {
  ...
  const fetchTasks = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_URL}/design-tasks`, {
      ...
      if (data.success) {
        ...
        console.error('Error fetching design tasks:', data.error);
      }
    } catch (error) {
      ...
      try {
        ...
        if (vendorData.success) {
          ...
          (vendorData.vendors || []).forEach((v: any) => {
            if (v.name && v.logo) map[v.name.trim().toLowerCase()] = v.logo;
          });
  ...
  // Fetch products and build SKU-to-vendor mapping
  useEffect(() => {
      ...
      try {
        ...
        if (data.success) {
          ...
          (data.items || []).forEach((product: any) => {
            ...
            if (sku && vendor) {
              // Get vendor logo from vendorLogoMap (will be populated by the time products are used)
              map[sku] = {
                vendorName: vendor,
                vendorLogo: undefined // Will be looked up from vendorLogoMap when rendering
              };
            }
          });
  ...
  const handleDeleteTask = async (taskId: string) => {
    try {
      const response = await fetch(`${API_URL}/design-tasks/${taskId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${publicAnonKey}` },
      });
      const data = await response.json();
