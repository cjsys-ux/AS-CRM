...
function KPICard({ label, value, icon: Icon, color, sub, trend, delay = 0 }: {
  ...
  return (
    ...
    >
      ...
      {sub && <p className="text-[11px] mt-1 text-slate-400">{sub}</p>}
    </motion.div>
  );
}
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          <button onClick={() => { setLoading(true); Promise.all([fetchAmazonOrders(), fetchCorporateInvoices()]).finally(() => setLoading(false)); }}
            className="px-3 py-2 text-sm font-medium text-slate-600 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 flex items-center gap-2 transition-all">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
        ...
        {[
          ...
          <button key={tab.id} onClick={() => setActiveSection(tab.id)}
            className={`relative px-5 py-2.5 rounded-xl text-sm font-semibold flex items-center gap-2 transition-all duration-300 ${
    ...
    <div className="space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
        ...
        <div className="bg-slate-800 px-6 py-4 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-white">Total Amount Due on</h2>
              <p className="text-slate-400 text-xs">Cost & profit breakdown</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <div className="bg-white text-slate-900 px-4 py-2 rounded-xl text-sm font-semibold shadow-sm">
              {displayDate}
            </div>
            <input
              type="date"
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                <th className="text-left px-5 py-3 font-semibold text-slate-500 text-[11px] uppercase tracking-wider whitespace-nowrap"></th>
                <th className="text-right px-5 py-3 font-semibold text-slate-500 text-[11px] uppercase tracking-wider whitespace-nowrap">Amount</th>
                <th className="text-right px-5 py-3 font-semibold text-slate-500 text-[11px] uppercase tracking-wider whitespace-nowrap">Total Cost</th>
                <th className="text-right px-5 py-3 font-semibold text-slate-500 text-[11px] uppercase tracking-wider whitespace-nowrap">Cost & Profit</th>
                <th className="text-right px-5 py-3 font-semibold text-slate-500 text-[11px] uppercase tracking-wider whitespace-nowrap">Payments</th>
                <th className="text-right px-5 py-3 font-semibold text-slate-500 text-[11px] uppercase tracking-wider whitespace-nowrap">Balance Due</th>
                <th className="text-right px-5 py-3 font-semibold text-slate-500 text-[11px] uppercase tracking-wider whitespace-nowrap min-w-[180px]">Total Balance Due w/ Profit</th>
                <th className="text-right px-5 py-3 font-semibold text-slate-500 text-[11px] uppercase tracking-wider whitespace-nowrap min-w-[170px]">Total Cost Only Due</th>
              </tr>
            </thead>
            <tbody>
              {[
                ...
                <tr key={row.label}
                  ...
                  <td className="px-5 py-3 text-right font-mono text-slate-700 whitespace-nowrap">$ {fmt(row.amount)}</td>
                  ...
                  <td className="px-5 py-3 text-right font-mono text-emerald-600 whitespace-nowrap">
                    ...
                    <>
                      <td rowSpan={4} className="px-5 py-3 text-center align-middle">
                        <div className="inline-flex flex-col items-center bg-slate-50 border border-slate-200 rounded-xl px-6 py-4">
                          <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-1">TOTAL DUE</span>
                          <span className="text-xl font-bold text-slate-900">$ {fmt(amountDue.totalBalanceWithProfit)}</span>
                        </div>
                      </td>
                      <td rowSpan={4} className="px-5 py-3 text-center align-middle">
                        <div className="inline-flex flex-col items-center gap-2">
                          <div className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 text-[11px] text-slate-400 font-medium whitespace-nowrap">
                            Product & Shipping Cost Only
                          </div>
                          <div className={`text-lg font-bold ${amountDue.totalCostOnlyDue < 0 ? 'text-emerald-600' : 'text-red-600'}`}>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
        ...
        <div className="bg-slate-800 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-sm font-semibold text-white">Amazon Product Revenue Summary</h3>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                <th className="text-left px-4 py-2.5 font-semibold text-slate-500 text-[11px] uppercase tracking-wider sticky left-0 bg-slate-50 z-10 min-w-[200px] whitespace-nowrap">Product Name</th>
                <th className="text-right px-3 py-2.5 font-semibold text-slate-500 text-[11px] uppercase tracking-wider whitespace-nowrap">Qty Sold</th>
                <th className="text-right px-3 py-2.5 font-semibold text-slate-500 text-[11px] uppercase tracking-wider whitespace-nowrap">Amazon Product Rev</th>
                <th className="text-right px-3 py-2.5 font-semibold text-slate-500 text-[11px] uppercase tracking-wider whitespace-nowrap">Amazon Shipping Rev</th>
                <th className="text-right px-3 py-2.5 font-semibold text-slate-500 text-[11px] uppercase tracking-wider whitespace-nowrap">Total Amazon Rev</th>
                <th className="text-right px-3 py-2.5 font-semibold text-slate-500 text-[11px] uppercase tracking-wider whitespace-nowrap">Product Cost</th>
                <th className="text-right px-3 py-2.5 font-semibold text-slate-500 text-[11px] uppercase tracking-wider whitespace-nowrap">Shipping Cost</th>
                <th className="text-right px-3 py-2.5 font-semibold text-slate-500 text-[11px] uppercase tracking-wider whitespace-nowrap">Total Cost</th>
                <th className="text-right px-3 py-2.5 font-semibold text-slate-500 text-[11px] uppercase tracking-wider whitespace-nowrap">Product Profit</th>
                <th className="text-right px-3 py-2.5 font-semibold text-slate-500 text-[11px] uppercase tracking-wider whitespace-nowrap">Shipping Profit</th>
                <th className="text-right px-3 py-2.5 font-semibold text-slate-500 text-[11px] uppercase tracking-wider whitespace-nowrap">Total Profit</th>
                <th className="text-right px-3 py-2.5 font-semibold text-slate-500 text-[11px] uppercase tracking-wider whitespace-nowrap">IPF Profit</th>
                <th className="text-right px-3 py-2.5 font-semibold text-slate-500 text-[11px] uppercase tracking-wider whitespace-nowrap">AS Profit</th>
              </tr>
            </thead>
            <tbody>
              {summary.map((p, idx) => (
                <tr key={p.productName}
                  ...
                  <td className="px-4 py-2 font-medium text-slate-800 sticky left-0 z-10 whitespace-nowrap" style={{ backgroundColor: idx % 2 === 0 ? '#fff' : '#f8fafc' }}>{p.productName}</td>
                  <td className="px-3 py-2 text-right font-mono text-slate-600 whitespace-nowrap">{fmtInt(p.quantitySold)}</td>
                  <td className="px-3 py-2 text-right font-mono text-slate-600 whitespace-nowrap">$ {fmt(p.amazonProductRevenue)}</td>
                  <td className="px-3 py-2 text-right font-mono text-slate-600 whitespace-nowrap">$ {fmt(p.amazonShippingRevenue)}</td>
                  <td className="px-3 py-2 text-right font-mono font-medium text-slate-800 whitespace-nowrap">$ {fmt(p.totalAmazonRevenue)}</td>
                  <td className="px-3 py-2 text-right font-mono text-slate-600 whitespace-nowrap">$ {fmt(p.productCost)}</td>
                  <td className="px-3 py-2 text-right font-mono text-slate-600 whitespace-nowrap">$ {fmt(p.shippingCost)}</td>
                  <td className="px-3 py-2 text-right font-mono font-medium text-slate-800 whitespace-nowrap">$ {fmt(p.totalCost)}</td>
                  ...
                  <td className={`px-3 py-2 text-right font-mono font-medium whitespace-nowrap ${p.totalProfit >= 0 ? 'text-emerald-700' : 'text-red-700'}`}>
                    $ {fmt(p.totalProfit)}
                  </td>
            ...
            <tfoot>
              <tr className="bg-slate-800 text-white font-medium">
                <td className="px-4 py-3 sticky left-0 z-10 font-semibold uppercase text-[11px] tracking-wider whitespace-nowrap" style={{ backgroundColor: '#1e293b' }}>Totals</td>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-sm font-semibold text-slate-800">Upcoming Payouts</h3>
            ...
            return (
              <div key={bucket.label}
                ...
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[11px] font-semibold uppercase tracking-wider text-slate-500">{bucket.label}</span>
    ...
    <div className="space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <h3 className="text-sm font-semibold text-slate-800 mb-1.5">AI Collection Insights</h3>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.25 }}
        ...
        <button onClick={() => setShowStatementModal(true)}
          className="px-4 py-2.5 text-sm font-semibold bg-slate-800 text-white rounded-xl hover:bg-slate-700 transition-all flex items-center gap-2">
              ...
              <tr className="bg-slate-50 border-b border-slate-200">
                {[
                  ...
                  <th key={col.key} onClick={() => handleSort(col.key)}
                    className="px-4 py-3 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider cursor-pointer hover:text-slate-900 select-none whitespace-nowrap">
              ...
              {filtered.map((inv: CorporateInvoice, idx: number) => {
                ...
                return (
                  <motion.tr key={inv.id}
                    ...
                    <td className="px-4 py-2.5 font-mono font-medium text-slate-700 whitespace-nowrap">{inv.invoiceNumber}</td>
                    ...
                    <td className="px-4 py-2.5 font-mono font-medium text-slate-800 whitespace-nowrap">${fmt(inv.amount)}</td>
                    ...
                    <td className="px-4 py-2.5 whitespace-nowrap">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-semibold border ${getStatusColor(inv.status)}`}>
      ...
      <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }}
        ...
        <div className="bg-slate-800 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-sm font-semibold text-white">Generate Collection Statement</h2>
          </div>
        ...
        <div className="p-6 space-y-4">
          <div>
            <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5 block">Sales Rep / Account Manager</label>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-center">
              <p className="text-[10px] font-semibold text-slate-400 uppercase">Outstanding</p>
              <p className="text-lg font-bold text-slate-800">${fmt(totalOwed)}</p>
            </div>
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-center">
              <p className="text-[10px] font-semibold text-slate-400 uppercase">Invoices</p>
              <p className="text-lg font-bold text-slate-800">{repInvoices.length}</p>
            </div>
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-center">
              <p className="text-[10px] font-semibold text-slate-400 uppercase">Past Due</p>
              <p className="text-lg font-bold text-slate-800">{pastDueCount}</p>
            </div>
          </div>
          ...
          <div className="flex justify-end gap-3 pt-2">
            <button onClick={onClose}
              className="px-4 py-2.5 text-sm font-medium text-slate-600 bg-slate-100 rounded-xl hover:bg-slate-200 transition-all">
              Cancel
            </button>
            <button onClick={() => { toast.success('Statement generated and ready to send!'); onClose(); }}
              className="px-6 py-2.5 text-sm font-semibold bg-slate-800 text-white rounded-xl hover:bg-slate-700 transition-all flex items-center gap-2">
