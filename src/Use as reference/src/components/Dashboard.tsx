...
import { IronBoundSafetyDashboard } from './IronBoundSafetyDashboard';
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      const [ordersRes, inventoryRes, customersRes, shipmentsRes, vendorsRes, posRes, pickRes, leadsRes, productionRes] = await Promise.all([
        fetch(`${API_URL}/orders`, { headers }).then(r => r.json()).catch(() => ({ orders: [] })),
        fetch(`${API_URL}/inventory`, { headers }).then(r => r.json()).catch(() => ({ items: [] })),
        fetch(`${API_URL}/customers`, { headers }).then(r => r.json()).catch(() => ({ customers: [] })),
        fetch(`${API_URL}/shipments`, { headers }).then(r => r.json()).catch(() => ({ shipments: [] })),
        fetch(`${API_URL}/vendors`, { headers }).then(r => r.json()).catch(() => ({ vendors: [] })),
        fetch(`${API_URL}/purchase-orders`, { headers }).then(r => r.json()).catch(() => ({ purchaseOrders: [] })),
        fetch(`${API_URL}/pick-lists`, { headers }).then(r => r.json()).catch(() => ({ pickLists: [] })),
        fetch(`${API_URL}/sales-leads`, { headers }).then(r => r.json()).catch(() => ({ leads: [] })),
        fetch(`${API_URL}/production`, { headers }).then(r => r.json()).catch(() => ({ orders: [] })),
      ]);
