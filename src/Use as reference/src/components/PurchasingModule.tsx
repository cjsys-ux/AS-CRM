    ...
    const fetchLogos = async () => {
      try {
        const [vendorRes, customerRes, contactRes] = await Promise.all([
          ...
          fetch(`${API_URL}/contacts`, { headers: { 'Authorization': `Bearer ${publicAnonKey}` } }),
        ]);
        ...
        if (vendorData.success) {
          ...
          (vendorData.vendors || []).forEach((v: any) => {
            ...
            if (vid) dataMap[vid] = { name: v.name || '', logo: v.logo || '', type: v.type || '' };
          });
      ...
      if (data.success) {
        ...
        if (newStatus === 'Confirmed') {
          try {
            ...
            const poRes = await fetch(`${API_URL}/purchase-orders/${orderId}`, {
              headers: { 'Authorization': `Bearer ${publicAnonKey}` },
            });
            const poData = await poRes.json();
            const po = poData.success ? poData.order : purchaseOrders.find(o => o.id === orderId);
            
            if (po) {
              // Check if an order already exists for this PO to prevent duplicates on status revert
              const existingOrdersRes = await fetch(`${API_URL}/orders`, {
                headers: { 'Authorization': `Bearer ${publicAnonKey}` },
              });
              const existingOrdersData = await existingOrdersRes.json();
              const existingOrder = existingOrdersData.success && (existingOrdersData.orders || []).find(
              ...
              if (existingOrder) {
                ...
                if (orderData.success) {
                  toast.success(`Order ${orderData.order.id} created from PO #${po.poNumber}!`);
                } else {
                  console.error('Error auto-creating order from confirmed PO:', orderData.error);
          ...
          try {
            ...
            if (ordersData.success) {
              ...
              for (const linkedOrder of linkedOrders) {
                const syncRes = await fetch(`${API_URL}/orders/${linkedOrder.id}`, {
                  method: 'PUT',
                  headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${publicAnonKey}`,
                ...
                if (syncData.success) {
                  ...
                  console.error('Error syncing status to linked order:', syncData.error);
                }
              }
            }
          } catch (syncErr) {
