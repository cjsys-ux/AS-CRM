            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="bg-slate-50 rounded-xl p-4 border border-slate-200 text-center">
                  <div className="text-3xl font-bold text-indigo-600">{totalLocations.toLocaleString()}</div>
...
export function WMSWarehousesTab() {
  ...
  const fetchLocations = useCallback(async (warehouseId: string) => {
    try {
      const res = await fetch(`${API_URL}/warehouse-locations/${warehouseId}`, { headers });
      const data = await res.json();
  ...
  const handleSaveWarehouse = async (data: any) => {
    try {
      ...
      const url = editWarehouse ? `${API_URL}/warehouses/${editWarehouse.id}` : `${API_URL}/warehouses`;
  ...
  const handleDeleteWarehouse = async (id: string) => {
    try {
      const res = await fetch(`${API_URL}/warehouses/${id}`, { method: 'DELETE', headers });
      const data = await res.json();
      if (data.success) {
  ...
  const handleGenerateLocations = async (locs: any[]) => {
    ...
    try {
      ...
      for (let i = 0; i < locs.length; i += 50) {
        const batch = locs.slice(i, i + 50);
        await fetch(`${API_URL}/warehouse-locations/${selectedWarehouse.id}`, {
  ...
  const handleDeleteLocation = async (loc: WarehouseLocation) => {
    try {
      const res = await fetch(`${API_URL}/warehouse-locations/${loc.warehouseId}/${loc.id}`, { method: 'DELETE', headers });
      const data = await res.json();
      if (data.success) {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      for (let i = 0; i < ids.length; i += 200) {
        ...
        const res = await fetch(`${API_URL}/warehouse-locations-bulk-delete`, {
          method: 'POST', headers,
          body: JSON.stringify({ warehouseId: selectedWarehouse.id, locationIds: batch }),
        });
        const data = await res.json();
        if (!data.success) {
          console.error('Bulk delete batch error:', data.error);
        }
      }
            ...
            <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-lg">
              <div className="flex items-center gap-4">
                <div className="flex-1 relative">
                  ...
                  <input type="text" placeholder="Search warehouses..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-12 pr-4 py-3 bg-slate-50 border-2 border-slate-200 rounded-xl text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all" />
                </div>
                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={fetchWarehouses} className="p-3 bg-slate-50 border-2 border-slate-200 rounded-xl hover:bg-slate-100 transition-colors" title="Refresh">
                  <RefreshCw className={`w-5 h-5 text-slate-600 ${loading ? 'animate-spin' : ''}`} />
                </motion.button>
                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => { setEditWarehouse(null); setDrawerOpen(true); }} className="flex items-center gap-2 px-5 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transition-all">
            ...
            {warehouses.length === 0 && !loading ? (
              <div className="bg-white rounded-2xl border border-slate-200 shadow-lg p-16 text-center">
                <div className="w-20 h-20 bg-gradient-to-br from-indigo-100 to-purple-100 rounded-2xl flex items-center justify-center mx-auto mb-4">
                  <Warehouse className="w-10 h-10 text-indigo-400" />
                </div>
                <h3 className="text-lg font-bold text-slate-900 mb-1">No Warehouses Yet</h3>
                <p className="text-sm text-slate-500 mb-4">Create your first warehouse to start managing locations and inventory.</p>
                <button onClick={() => { setEditWarehouse(null); setDrawerOpen(true); }} className="px-5 py-3 bg-indigo-600 text-white font-semibold rounded-xl hover:bg-indigo-700">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {warehouses.filter(w => w.name.toLowerCase().includes(search.toLowerCase()) || w.code?.toLowerCase().includes(search.toLowerCase())).map((wh, i) => (
                  <motion.div key={wh.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="bg-white rounded-2xl border border-slate-200 shadow-lg hover:shadow-xl transition-all overflow-hidden group">
                    {/* Gradient Header */}
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <span className="text-indigo-200 text-sm font-mono">{wh.code}</span>
                          </div>
                        </div>
                    ...
                    {/* Card Body */}
                    <div className="p-5">
                      {wh.address && <p className="text-sm text-slate-600 mb-3 flex items-start gap-1.5"><MapPin className="w-3.5 h-3.5 text-slate-400 mt-0.5 shrink-0" />{wh.address}{wh.city ? `, ${wh.city}` : ''}{wh.state ? `, ${wh.state}` : ''} {wh.zip}</p>}

                      {/* Primary Stats Row */}
                      <div className="grid grid-cols-3 gap-2 mb-3">
                        <div className="bg-slate-50 rounded-xl p-3 text-center border border-slate-100">
                          <div className="text-lg font-bold text-slate-900">{wh.totalArea ? `${(wh.totalArea / 1000).toFixed(0)}K` : '—'}</div>
                          <div className="text-[10px] font-semibold text-slate-500 uppercase tracking-wide">Sq. Ft.</div>
                        </div>
                        <div className="bg-slate-50 rounded-xl p-3 text-center border border-slate-100">
                          <div className="text-lg font-bold text-slate-900">{wh.totalLocations || 0}</div>
                          <div className="text-[10px] font-semibold text-slate-500 uppercase tracking-wide">Locations</div>
                        </div>
                        <div className="bg-emerald-50 rounded-xl p-3 text-center border border-emerald-100">
                          <div className="text-lg font-bold text-emerald-700">{(wh.totalValue || 0) > 0 ? ((wh.totalValue || 0) >= 1000 ? `$${((wh.totalValue || 0) / 1000).toFixed(1)}K` : `$${Math.round(wh.totalValue || 0).toLocaleString()}`) : '$0'}</div>
                      ...
                      <div className="grid grid-cols-3 gap-2 mb-4">
                        <div className="bg-blue-50 rounded-xl p-2.5 text-center border border-blue-100">
                          <div className="text-base font-bold text-blue-700">{wh.openPalletLocations || 0}</div>
                        ...
                        <div className="bg-purple-50 rounded-xl p-2.5 text-center border border-purple-100">
                          <div className="text-base font-bold text-purple-700">{wh.totalBinLocations || 0}</div>
                        ...
                        <div className="bg-indigo-50 rounded-xl p-2.5 text-center border border-indigo-100">
                          <div className="text-base font-bold text-indigo-700">{wh.totalProducts || 0}</div>
                          <div className="text-[10px] font-semibold text-indigo-500 uppercase tracking-wide">Products</div>
                        </div>
                      </div>

                      {wh.manager && <div className="text-xs text-slate-500 mb-3">Manager: <span className="font-semibold text-slate-700">{wh.manager}</span></div>}

                      {/* Actions */}
                      <div className="flex items-center gap-2">
                        <button onClick={() => openWarehouseDetail(wh)} className="flex-1 px-4 py-2.5 bg-indigo-50 text-indigo-700 font-semibold rounded-xl text-sm hover:bg-indigo-100 transition-colors flex items-center justify-center gap-2">
                          <Eye className="w-4 h-4" />Manage
                        </button>
                        <button onClick={() => { setEditWarehouse(wh); setDrawerOpen(true); }} className="p-2.5 border border-slate-200 rounded-xl hover:bg-slate-50">
                          <Edit2 className="w-4 h-4 text-slate-500" />
                        </button>
                        <button onClick={() => handleDeleteWarehouse(wh.id)} className="p-2.5 border border-red-200 rounded-xl hover:bg-red-50">
                  ...
                  {filteredLocations.length === 0 ? (
                    <tr>
                      <td colSpan={8} className="px-8 py-16 text-center">
                        ...
                        <h3 className="text-lg font-bold text-slate-900 mb-1">No Locations</h3>
