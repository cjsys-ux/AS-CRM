...
function CreateShipmentDrawer({
  ...
  const fetchCompanies = async () => {
    ...
    try {
      const [custRes, vendorRes] = await Promise.all([
        fetch(`${API_URL}/customers`, { headers: headers_json }),
        fetch(`${API_URL}/vendors`, { headers: headers_json }),
      ]);
  ...
  const handleCompanyChange = async (companyId: string) => {
    ...
    if (shipToType === 'customer') {
      ...
      // Fetch vendor contacts from dedicated endpoint
      try {
        const res = await fetch(`${API_URL}/vendors/${companyId}/contacts`, { headers: headers_json });
  ...
  const handleCreateShipment = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/pick-lists`, {
        method: 'POST',
      ...
      if (result.success) {
        ...
        await fetch(`${API_URL}/inventory/${item.id}`, {
          method: 'PUT',
          headers: headers_json,
          body: JSON.stringify({ onOrder: newOnOrder }),
        });
