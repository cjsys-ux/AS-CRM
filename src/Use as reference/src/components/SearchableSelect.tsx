...
export interface SelectOption {
  ...
  image?: string; // Customer logo URL
}
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className={`w-full px-3.5 py-2 bg-slate-50 border rounded-xl text-sm text-left font-medium transition-all flex items-center justify-between gap-2 ${
          isOpen
        ...
        <div className="flex items-center gap-2.5 min-w-0 flex-1">
          {selectedOption?.image ? (
            <div className="w-8 h-7 rounded-md flex items-center justify-center overflow-hidden border border-slate-200 bg-white shrink-0">
              <img src={selectedOption.image} alt={selectedOption.label} className="max-w-full max-h-full object-contain p-0.5" />
            </div>
          ) : selectedOption?.icon ? (
                ...
                filtered.map(option => {
                  ...
                  return (
                    ...
                    >
                      {option.image ? (
                        <div className="w-8 h-7 rounded-md flex items-center justify-center overflow-hidden border border-slate-200 bg-white shrink-0">
                          <img src={option.image} alt={option.label} className="max-w-full max-h-full object-contain p-0.5" />
                        </div>
                      ) : option.icon ? (
