...
const SETTINGS_API_URL = `https://${projectId}.supabase.co/functions/v1/make-server-c0840c88`;
...
export function GeneralSettingsPage() {
  ...
  useEffect(() => {
    const fetchPipelineChecklists = async () => {
      try {
        const res = await fetch(`${SETTINGS_API_URL}/settings/pipeline-checklists`, {
  ...
  const savePipelineChecklists = useCallback(async (updatedModules: Module[]) => {
    ...
    try {
      await fetch(`${SETTINGS_API_URL}/settings/pipeline-checklists`, {
        method: 'PUT',
        headers: {
  ...
  useEffect(() => {
    const fetchRoles = async () => {
      try {
        const res = await fetch(`${SETTINGS_API_URL}/settings/user-roles`, {
          headers: { 'Authorization': `Bearer ${publicAnonKey}` },
        });
        const data = await res.json();
  ...
  const saveUserRoles = useCallback(async (updatedModules: Module[]) => {
    ...
    try {
      await fetch(`${SETTINGS_API_URL}/settings/user-roles`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${publicAnonKey}`,
          'Content-Type': 'application/json',
  ...
  const fetchCarrierAccounts = async () => {
    ...
    try {
      const response = await fetch(`${SETTINGS_API_URL}/carrier-accounts`, {
        headers: { 'Authorization': `Bearer ${publicAnonKey}` },
      });
  ...
  const handleUpdateCarrierAccount = async (id: string) => {
    try {
      const response = await fetch(`${SETTINGS_API_URL}/carrier-accounts/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${publicAnonKey}` },
        body: JSON.stringify(editCarrierForm),
      });
      const data = await response.json();
      if (data.success) {
