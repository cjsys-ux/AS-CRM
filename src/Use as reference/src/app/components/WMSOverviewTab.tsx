  ...
  const loadStats = async () => {
    try {
      ...
      const [whData, locData, invData, rcvData, pickData] = await Promise.all([
        fetchSafe(`${API_URL}/warehouses`),
        fetchSafe(`${API_URL}/warehouse-locations-all`),
        fetchSafe(`${API_URL}/inventory`),
        fetchSafe(`${API_URL}/receiving`),
        fetchSafe(`${API_URL}/pick-lists`),
      ]);
