...
export function OrderDetailView({ order: initialOrder, onBack, onEdit, onRefresh }: OrderDetailViewProps) {
  ...
  useEffect(() => {
    const syncFromPO = async () => {
      ...
      try {
        const poRes = await fetch(`${API_URL}/purchase-orders/${order.sourcePOId}`, {
          headers: { 'Authorization': `Bearer ${publicAnonKey}` },
        });
        const poData = await poRes.json();
        if (!poData.success || !poData.order) return;
        const po = poData.order;
      ...
      try {
        ...
        if (data.success) {
          const vendor = (data.vendors || []).find((v: any) =>
            v.name?.trim().toLowerCase() === order.vendor?.trim().toLowerCase() ||
  ...
  useEffect(() => {
    const checkReceiving = async () => {
      try {
        ...
        const data = await res.json();
        if (data.success) {
          const linked = data.receipts.find((r: any) => r.sourceOrderId === order.id);
  ...
  useEffect(() => {
    const fetchLinkedShipments = async () => {
      try {
        ...
        const data = await res.json();
        if (data.success) {
          const linked = (data.shipments || []).filter((s: any) =>
            s.poNumber === order.id || s.poId === order.id ||
            s.orderNumber === order.id || s.poNumber === order.sourcePONumber
          );
  ...
  const updateOrderField = async (updates: Partial<Order>) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Notify if a receiving entry was auto-created
        if (data.receivingCreated && data.receivingId) {
          setLinkedReceivingId(data.receivingId);
          toast.success(`Receiving entry ${data.receivingId} created in Warehouse — shipment is now being tracked`, {
            duration: 5000,
          });
        }
        ...
        if (updates.status && ['Shipped', 'Delivered'].includes(updates.status) && order.sourcePOId) {
          try {
            ...
            if (poData.success) {
              ...
              console.error('Error syncing status to linked PO:', poData.error);
            }
          } catch (syncErr) {
