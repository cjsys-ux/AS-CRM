    ...
    try {
      ...
      if (data.success && data.contacts) {
        const mapped: Contact[] = data.contacts.map((c: any) => ({
          ...
          phone: c.phone || '',
          company: c.company || '',
          role: c.jobTitle || c.role || '',
          jobTitle: c.jobTitle || '',
          contactType: c.contactType || '',
          address: [c.city, c.state].filter(Boolean).join(', ') || '',
          fullAddress: [c.addressLine1, c.addressLine2, c.city, c.state, c.zipCode, c.country].filter(Boolean).join(', ') || '',
        }));
    ...
    if (addToContactsModule) {
      ...
      try {
        ...
        if (data.success) {
          ...
          // Update the contact ID with the server-generated one
          contact.id = data.contact?.id || contact.id;
        } else {
          console.error('Error saving contact to module:', data.error);
