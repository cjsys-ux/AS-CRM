...
const PO_API_URL = `https://${projectId}.supabase.co/functions/v1/make-server-c0840c88`;
...
export function PurchaseOrderDetailView({ order, onBack, onEdit, onStatusChange, onOrderUpdate }: PurchaseOrderDetailViewProps) {
  ...
  const fetchVendorPaymentTerms = async (vendorName: string, forceAddressRefresh = false) => {
    try {
      const response = await fetch(`${PO_API_URL}/vendors`, {
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${publicAnonKey}` },
      });
      const data = await response.json();
      if (data.success && data.vendors) {
        const matchedVendor = data.vendors.find((v: any) => 
          v.name?.toLowerCase() === vendorName.toLowerCase() || 
          v.vendorName?.toLowerCase() === vendorName.toLowerCase()
        );
        if (matchedVendor) {
          const terms = matchedVendor.paymentTerms || matchedVendor.netTerms || '';
          if (terms && !paymentTerms) {
            setPaymentTerms(terms);
          }
          // Store all vendor addresses for the editor
          const addresses = matchedVendor.addresses || [];
          setVendorAllAddresses(addresses);
          // Default vendor address/contact from primary address (or first address)
          if (addresses.length > 0 && (!vendorAddress || forceAddressRefresh)) {
            const primary = addresses.find((a: any) => a.isPrimary) || addresses[0];
            setVendorAddress(primary);
            setVendorContactPerson(primary.contactPerson || '');
          }
        }
      }
    } catch (err) {
      console.error('Error fetching vendor payment terms:', err);
    }
  };
  ...
  const saveFieldsToServer = async (fields: Record<string, any>) => {
    try {
      const response = await fetch(`${PO_API_URL}/purchase-orders/${order.id}`, {
        method: 'PUT',
        headers: {
      ...
      const data = await response.json();
      if (!data.success) {
        console.error('Error auto-saving PO field:', data.error);
      }
    } catch (err) {
      console.error('Error auto-saving PO field:', err);
    }
  };
  ...
  useEffect(() => {
    if (isInitialMount.current) return;
    saveFieldsToServer({ status });
  }, [status]);

  // Fetch carrier accounts from settings
  useEffect(() => {
    const fetchCarrierAccounts = async () => {
      try {
        const response = await fetch(`${PO_API_URL}/carrier-accounts`, {
          headers: { 'Authorization': `Bearer ${publicAnonKey}` },
        });
        const data = await response.json();
        if (data.success && data.accounts) {
          setCarrierAccountOptions(data.accounts.map((a: any) => ({
            id: a.id,
            carrier: a.carrier || '',
            accountNumber: a.accountNumber || '',
            label: a.label || `${a.carrier} - ${a.accountNumber}`,
          })));
        }
      } catch (err) {
  ...
  // Fetch pipeline products for Product Name dropdown
  useEffect(() => {
    const fetchPipelineProducts = async () => {
      try {
        const response = await fetch(`${PO_API_URL}/products`, {
          headers: { 'Authorization': `Bearer ${publicAnonKey}` },
        });
        const data = await response.json();
        if (data.success && data.products) {
          setPipelineProducts(data.products.map((p: any) => ({
            id: p.id,
            name: p.name || '',
            projectNumber: p.projectNumber || '',
            sku: p.internalSKU || p.sku || '',
            vendor: p.vendor || '',
          })));
        }
      } catch (err) {
        console.error('Error fetching pipeline products:', err);
      }
    };
  ...
  const handleStatusChange = (newStatus: string) => {
    ...
    if (newStatus === 'Shipped') {
      ...
      // Don't set status yet - wait for shipment creation confirmation
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-slate-100">
                        ...
                        {['Net 15', 'Net 30', 'Net 45', 'Net 60', 'Due on Receipt', 'Prepaid', 'COD', '50% Deposit'].map(term => (
                          <button
                            ...
                            onClick={async () => {
                              ...
                              // Save payment terms to PO on the server
                              try {
                                await fetch(`${PO_API_URL}/purchase-orders/${order.id}`, {
                                  method: 'PUT',
                                  headers: {
                                    'Content-Type': 'application/json',
                                    'Authorization': `Bearer ${publicAnonKey}`,
                                  },
                                  body: JSON.stringify({ paymentTerms: term }),
                                });
                              } catch (err) {
      ...
      <CreateShipmentFromPOModal
        ...
        onConfirm={(shipments) => {
          setShowShipmentModal(false);
          // Set status to Shipped now that shipment was successfully created
          setStatus('Shipped');
