...
// ── CORS must be registered first so ALL responses (including errors & 404s) carry CORS headers ──
app.use(
  "/*",
  cors({
    origin: "*",
...
// Update file metadata (rename, category)
app.patch("/make-server-c0840c88/products/:productId/files/:fileId", async (c) => {
  try {
    const productId = c.req.param("productId");
    const fileId = c.req.param("fileId");
    const updates = await c.req.json();
...
app.post("/make-server-c0840c88/products/:productId/chat", async (c) => {
  try {
    ...
    if (contentType?.includes('multipart/form-data')) {
      ...
      userName = (formData.get('user') as string) || 'Current User';
      const file = formData.get('file') as File;
      ...
      userName = body.user || 'Current User';
    }
...
app.post("/make-server-c0840c88/shipments", async (c) => {
  try {
    ...
    console.log("Creating shipment with payload keys:", Object.keys(shipment), "poNumber:", shipment.poNumber, "carrier:", shipment.carrier);
...
app.post("/make-server-c0840c88/productdb", async (c) => {
  try {
    ...
    // Safety: strip raw base64 from image fields before storing
    if (Array.isArray(product.productImages)) {
      product.productImages = product.productImages.map((img: string) =>
    ...
    console.log(`Product database item created: productdb:${product.id} (name: ${product.name})`);
    return c.json({ success: true, product });
  } catch (error) {
...
app.put("/make-server-c0840c88/productdb/:id", async (c) => {
  try {
    ...
    // Safety: strip raw base64 data URIs from productImages to prevent oversized KV entries.
    // Images should be uploaded to Supabase Storage first (via /upload/product-image).
    if (Array.isArray(updates.productImages)) {
      updates.productImages = updates.productImages.map((img: string) =>
        typeof img === "string" && img.startsWith("data:") ? "" : img
      ).filter(Boolean);
    }
    if (typeof updates.image === "string" && updates.image.startsWith("data:")) {
      updates.image = ""; // Don't store raw base64 as primary image
    }

    // Require name — prevents products from becoming invisible (filtered by p.name)
    if (updates.name !== undefined && !String(updates.name).trim()) {
      return c.json({ success: false, error: "Product name cannot be empty" }, 400);
    }
    ...
    console.log(`Product database item updated: productdb:${id} (name: ${updated.name})`);
    return c.json({ success: true, product: updated });
  } catch (error) {
...
// Upload a product image (base64 data URI) to Supabase Storage and return a signed URL
app.post("/make-server-c0840c88/upload/product-image", async (c) => {
  try {
    await ensureBucket();
    const { imageBase64 } = await c.req.json();
    if (!imageBase64) {
    ...
    if (typeof imageBase64 === "string" && imageBase64.startsWith("http")) {
      return c.json({ success: true, url: imageBase64 });
    }

    const matches = (imageBase64 as string).match(/^data:([^;]+);base64,(.+)$/);
    if (!matches) {
      return c.json({ success: false, error: "Invalid image format (expected data URI)" }, 400);
    }

    const contentType = matches[1];
    const base64Data = matches[2];
    const ext = contentType.split("/")[1]?.split("+")[0] || "png";
    const storageName = `product-img-${Date.now()}-${Math.random().toString(36).substring(2, 8)}.${ext}`;
    ...
    if (signError) {
      console.error("Product image signed URL error:", signError);
      return c.json({ success: false, error: `Signed URL failed: ${signError.message}` }, 500);
    }

    return c.json({ success: true, url: signedData?.signedUrl, path: data.path });
  } catch (error) {
