...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const result = await db.collection('amazon_orders').updateOne(filter, { $set: setPayload });
    if (result.matchedCount === 0) return res.status(404).json({ error: 'Amazon order not found.' });
    return res.status(200).json({ success: true });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Failed to update Amazon order.';
