...
const ALLOWED_FIELDS = ['name', 'type', 'color', 'notes'];
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const result = await db.collection('warehouse_zones').updateOne(filter, { $set: setPayload });
    if (result.matchedCount === 0) return res.status(404).json({ error: 'Zone not found.' });
